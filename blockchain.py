...
class Blockchain:
    ...
    def build_new_block(self):
        ...
        #1. pick 5 validators based on stake, stake age and randomness from ValidatorList
        ...
        print()
        #2. pick 10 miners based on mistakes and application age from MinerList
        ...
        #4. make "ALL" the miners mine the block
            #We get a generator with the next(generator_for_miners) being the next
        ...
        for place in range(1, 11):
            #winner_miner is a tuple with (integer proof from proof_of_work, miner_object)
        ...
        #6. make the five random validators check the proof with the broadcastBlock function
            block_has_consensus = broadcastBlock(five_validator_winners,self.last_block_hash(), new_hash, winner_miner[0])
            #We get two lists with validators that approved the block and dissaproved
        ...
        #8. if 3 yes 2 no, it passes, block reward is shared by 3 and 2 no's gets slashed. if 2 yes 3 no, 2 yes's lose 0.6% of their block reward. Continues till there is consensus
            if block_has_consensus[0]:
                ...
                #Give reward to validators that said yes and are in majority
                for reward_validator in validators_said_yes:
                    reward_validator.get_reward_for_block(self.block_reward)
                print()
                #Punish validators that said no and are in minority
                ...
                #System gets 10% of the block reward
                self.system_reward += self.block_reward * 0.10
                print("System gets 10% of the block reward which is", self.block_reward * 0.10)
                
                #Reset stake age for validators
                ...
                #Clear transactions making room for new transactions
                self.transactions_per_block.clear()
                #update block reward if needed
